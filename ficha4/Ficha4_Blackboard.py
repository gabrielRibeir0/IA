...
def minimax(estado, profundidade, jogador):
    ...
    if jogador == BOT :
        melhor_movimento = [-1, -1, -infinito] 
    else :
        melhor_movimento = [-1, -1, +infinito]
    
    for celula in celulas_vazias(estado):
        estado[celula[0]][celula[1]] = jogador
        movimento = minimax(estado, profundidade-1, -jogador)
        estado[celula[0]][celula[1]] = 0
        movimento[0], movimento[1] = celula[0], celula[1]

        if jogador == BOT:
            if movimento[2] > melhor_movimento[2]:
...
def minimax_iter(estado, profundidade, jogador):
    melhor_movimento = []
    if profundidade == 0 or verifica_vitoria(estado, HUMANO) or verifica_vitoria(estado, BOT):
        score = avaliar(estado)
        return [-1, -1, score]
